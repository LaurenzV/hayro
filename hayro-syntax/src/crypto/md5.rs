...
pub(crate) fn calculate(data: &[u8]) -> [u8; 16] {
    ...
    for chunk in message.chunks_exact(64) {
        ...
        for i in 0..64 {
            let (f, g) = match i {
                0..=15 => ((working_vars[1] & working_vars[2]) | (!working_vars[1] & working_vars[3]), i),
                16..=31 => ((working_vars[3] & working_vars[1]) | (!working_vars[3] & working_vars[2]), (5 * i + 1) % 16),
                32..=47 => (working_vars[1] ^ working_vars[2] ^ working_vars[3], (3 * i + 5) % 16),
                48..=63 => (working_vars[2] ^ (working_vars[1] | !working_vars[3]), (7 * i) % 16),
                _ => unreachable!(),
            };
...
mod tests {
    ...
    fn test_block_boundaries() {
        assert_eq!(
            ...
            [0xc9, 0xcc, 0xf1, 0x68, 0x91, 0x4a, 0x1b, 0xcf, 0xc3, 0x22, 0x9f, 0x19, 0x48, 0xe6, 0x7d, 0xa0]
        );
        assert_eq!(
            calculate(b"1234567890123456789012345678901234567890123456789012345678901234"),
            [0xeb, 0x6c, 0x41, 0x79, 0xc0, 0xa7, 0xc8, 0x2c, 0xc2, 0x82, 0x8c, 0x1e, 0x63, 0x38, 0xe1, 0x65]
        );
    }
    ...
    fn test_binary_and_special_data() {
        assert_eq!(
            calculate(&[0u8; 100]),
            [0x6d, 0x0b, 0xb0, 0x09, 0x54, 0xce, 0xb7, 0xfb, 0xee, 0x43, 0x6b, 0xb5, 0x5a, 0x83, 0x97, 0xa9]
        );
        assert_eq!(
        ...
        assert_eq!(
            calculate("🦀💎".as_bytes()),
            [0xbc, 0xab, 0xb9, 0x04, 0x18, 0xab, 0x44, 0xce, 0xca, 0xa9, 0x53, 0xd9, 0x73, 0x9f, 0x85, 0x0c]
        );
    }
    ...
    fn test_large_and_repetitive() {
        assert_eq!(
            ...
            [0x42, 0x70, 0x08, 0xb3, 0xfe, 0x19, 0x2f, 0x66,
                0x3d, 0x66, 0x5f, 0x56, 0xcd, 0x75, 0x71, 0x6c]
        );
        assert_eq!(
        ...
        assert_eq!(
            ...
            [0x97, 0xac, 0x82, 0xa5, 0xb8, 0x25, 0x23, 0x9e,
                0x78, 0x2d, 0x03, 0x39, 0xe2, 0xd7, 0xb9, 0x10]
        );
    }
}